...
public class DashedDrawer implements IDrawer
	{
		...
		public function lineTo(x:Number, y:Number):void {
			if(_alignToCorners && !_isAligned){
				initDashAlign(this.lineLength(x-penX, y-penY));
				lineTo(x, y);
				endDashAlign();
				return;
			}
		...
		public function curveTo(cx:Number, cy:Number, x:Number, y:Number):void {
			if(_alignToCorners && !_isAligned){
				initDashAlign(this.curveLength(penX, penY, cx, cy, x, y, _curveAccuracy));
				curveTo(cx, cy, x, y);
				endDashAlign();
				return;
			}
			...
			do {
				var curveLength:Number = this.curveLength(penX, penY, cx, cy, x, y, _curveAccuracy);
